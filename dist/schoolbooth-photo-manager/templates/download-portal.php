<div class="schoolbooth-portal">
    ...
    $access_code = isset($_GET['code']) ? schoolbooth_normalize_access_code(sanitize_text_field($_GET['code'])) : '';
    
    if (!$access_code): 
        // Step 1: Access Code Entry
        ?>
        <div class="access-form">
            <h2><?php _e('Access Your Photos', 'schoolbooth'); ?></h2>
            <form method="get">
                <input type="hidden" name="page_id" value="<?php echo get_the_ID(); ?>">
                
                <div class="form-group">
                    <label for="code"><?php _e('Access Code:', 'schoolbooth'); ?></label>
                    <input type="text" id="code" name="code" required placeholder="<?php esc_attr_e('Enter your access code', 'schoolbooth'); ?>">
                </div>
                
                <button type="submit"><?php _e('View Photos', 'schoolbooth'); ?></button>
            </form>
        </div>
    <?php else:
        ...
        $form_completed = SCHOOLBOOTH_Permissions_Form_Handler::has_completed_form($access_code);
        
        if (!$form_completed):
            // Show permissions form
            echo SCHOOLBOOTH_Permissions_Form_Handler::render_form($access_code);
        else:
            ...
            if (!empty($photos)): ?>
                <div class="photos-section">
                    <h2><?php _e('Your Photos', 'schoolbooth'); ?></h2>
                    
                    <?php if (count($photos) > 1): ?>
                        <div class="photo-actions">
                            <button class="download-all"><?php _e('Download All', 'schoolbooth'); ?></button>
                            <?php if ($can_delete): ?>
                                <button class="delete-all"><?php _e('Delete All', 'schoolbooth'); ?></button>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="photo-grid">
                        <?php foreach ($photos as $photo): ?>
                            ...
                            $share_text = sprintf(__('View my Schoolbooth photo: %s', 'schoolbooth'), $photo['label']);
                            ?>
                                    ...
                                    <div class="share-links">
                                        <a class="share-link" target="_blank" rel="noopener"
                                           href="https://www.facebook.com/sharer/sharer.php?u=<?php echo rawurlencode($share_url); ?>">
                                            <?php _e('Facebook', 'schoolbooth'); ?>
                                        </a>
                                        <a class="share-link" target="_blank" rel="noopener"
                                           href="https://twitter.com/intent/tweet?url=<?php echo rawurlencode($share_url); ?>&text=<?php echo rawurlencode($share_text); ?>">
                                            <?php _e('X', 'schoolbooth'); ?>
                                        </a>
                                        <a class="share-link" target="_blank" rel="noopener"
                                           href="mailto:?subject=<?php echo rawurlencode(__('Schoolbooth Photo Link', 'schoolbooth')); ?>&body=<?php echo rawurlencode($share_text . ' ' . $share_url); ?>">
                                            <?php _e('Email', 'schoolbooth'); ?>
                                        </a>
                                        <button type="button" class="share-link copy-link-btn"
                                                data-share-url="<?php echo esc_attr($share_url); ?>">
                                            <?php _e('Copy Link', 'schoolbooth'); ?>
                                        </button>
                                    </div>
                                </div>
                            </div>
            ...
            <?php else: ?>
                <div class="no-photos">
                    <p><?php _e('No photos found for this access code', 'schoolbooth'); ?></p>
                    <p><?php _e('The photos may have expired or all download limit has been reached.', 'schoolbooth'); ?></p>
                    <a href="<?php echo esc_url(remove_query_arg(['code', 'file'])); ?>" class="btn">
                        <?php _e('Try Another Code', 'schoolbooth'); ?>
                    </a>
                </div>
            <?php endif;
        endif;
